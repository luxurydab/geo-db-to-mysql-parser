#!/bin/sh
# GeoNames cities5000.zip database into MySQL
#all cities with a population > 5000 or PPLA (ca 40.000)
#table structure
...
/usr/bin/mysql $@ --local-infile=1  -e "USE $database; TRUNCATE $table; LOAD DATA LOCAL INFILE '/tmp/cities5000.txt' INTO TABLE $table FIELDS TERMINATED BY \"\t\" LINES TERMINATED BY \"\n\" (geonameid, name, asciiname, alternatenames, latitude, longitude, feature_class, feature_code, country_code, cc2, admin1_code, admin2_code, admin3_code, admin4_code, population, elevation, dem, timezone, modification_date);"
